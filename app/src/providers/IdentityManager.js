...
function useResolveLocalIdentity() {
  const api = useApi();
  return useCallback(address => api.resolveAddressIdentity(address).toPromise(), [api]);
}

// Request a modification of the local identity
function useModifyLocalIdentity() {
  const api = useApi();
  return useCallback(address => api.requestAddressIdentityModification(address).toPromise(), [api]);
}
...
export const IdentityProvider = ({ children }) => {
  return <IdentityContext.Provider value={{ updates$ }}>{children}</IdentityContext.Provider>;
};
